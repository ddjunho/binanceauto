...
def predict_target_price(ticker, target_type):
    ...
    df = df.astype({'open': 'float', 'high': 'float', 'low': 'float', 'close': 'float', 'volume': 'float', 'quote_asset_volume': 'float', 'taker_buy_base': 'float', 'taker_buy_quote': 'float', 'ignored': 'float'})
    print(df.dtypes)
    # 입력 데이터 전처리
    X = df
    ...
    # 새로운 데이터에 대한 예측
    last_data = df[['time', 'open', 'high', 'low', 'close', 'volume', 'close_time', 'quote_asset_volume', 'trades', 'taker_buy_base', 'taker_buy_quote']].iloc[-data:].values
